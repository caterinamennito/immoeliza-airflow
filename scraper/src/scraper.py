...
class Scraper:
    ...
    def fetch_immo_list(self, start_page=1, max_pages=50, max_workers=5, property_type="apartment"):
        """
    ...
    def fetch_details_soup_multithread(self, property_type="apartment"):
        # read from links.csv
        try:
        ...
        key_list = [
                    "url", "address", "price", "State of the property", "Build Year", "Availability", "Number of bedrooms", "Surface bedroom 1", "Surface bedroom 2", "Surface bedroom 3", "Livable surface", "Furnished", "Surface of living-room", "Cellar", "Diningroom", "Surface of the diningroom", "Kitchen equipment", "Surface kitchen", "Number of bathrooms", "Number of showers", "Number of toilets", "Type of heating", "Type of glazing", "Entry phone", "Elevator", "Access for disabled", "Orientation of the front facade", "Floor of appartment", "Number of facades", "Number of floors", "Garden", "Surface garden", "Terrace", "Surface terrace", "Total land surface", "Sewer Connection", "Gas", "Running water", "Swimming pool", "Specific primary energy consumption", "Validity date EPC/PEB", "CO2 emission", "Certification - Electrical installation", "Flooding Area type", "Demarcated flooding area", "postal_code", "city"
                ]
        try:
            existing_df = pd.read_csv(f"{property_type}_data.csv")
        except FileNotFoundError:
            # if file not found, create empty dataframe
            print(f"{property_type}_data.csv not found, creating a new one")
        ...
        if results:
            try:
                df_result = pd.concat(
                    [existing_df, pd.DataFrame(results)], ignore_index=True
                )
            except FileNotFoundError:
